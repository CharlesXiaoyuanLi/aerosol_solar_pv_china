...
for i in range(len(pro_nm)):
    color[pro_nm[i]] = sfc_diffuse_aer_ratio[i]/100.
color['Shanghai'] = color['Jiangsu']

cmap=cm.RdYlBu_r

#bounds = np.linspace(-50,0,11)
bounds = np.linspace(1.5,4.5,7)
...
for key, value in patches.iteritems():
    ...
    ax.add_collection(PatchCollection(value,\
        facecolor=cm.RdYlBu_r(norm(color[key])),\
        cmap=cm.RdYlBu_r, edgecolor='k', linewidth=1., zorder=2))
...
cb.set_label('times increased')

plt.savefig('prov_sfc_sw_diffuse_increase_aer_ratio.ps',format='ps')
